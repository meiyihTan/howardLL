...
gray_dir = './dataset/IC15_004/test/gray/'
...
with torch.no_grad():
    ...
    for test_id in test_ids:
        ...
        gray_path = gray_dir + '%d.png' % test_id
        ...
        input_gray_img = cv2.imread(gray_path, cv2.IMREAD_GRAYSCALE)
        input_gray_full = np.expand_dims(np.expand_dims(np.float32(input_gray_img / 255.0), axis=2), axis=0)

        input_full = np.minimum(input_full, 1.0)
        in_img = torch.from_numpy(input_full).permute(0,3,1,2).to(device)
        in_gray_img = torch.from_numpy(input_gray_full).permute(0,3,1,2).to(device)
